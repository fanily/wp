	...
	public static function text_area( $attributes = array() ){
		...
		switch( $attributes['format'] ){
			...
			default:
					...
					$html .= "<textarea ".self::format_attributes( array('id','name','class','style'), array(), $attributes )." ".$attributes['extra'].">".esc_textarea( otw_stripslashes( $attributes['value'] ) )."</textarea>";
	...
	public static function format_attribute( $attribute_name, $attribute_key, $attributes, $show_empty = false, $element_type = '' ){
		...
		if( isset( $attributes[ $attribute_key ] ) && strlen( trim( $attributes[ $attribute_key ] ) ) ){
			if( in_array( $element_type, array( 'text_input' ) ) && $attribute_key == 'value' ){
				return ' '.$attribute_name.'="'.esc_attr( otw_htmlentities( otw_stripslashes( $attributes[ $attribute_key ] ) ) ).'"';
			}else{
				return ' '.$attribute_name.'="'.esc_attr( $attributes[ $attribute_key ] ).'"';
			}
		}elseif( $show_empty ){
